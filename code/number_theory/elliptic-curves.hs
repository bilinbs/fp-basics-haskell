...
addEllP (EllpCurve a b) (Point x1 y1) (Point x2 y2) p 
                | x1 == x2 && y1 == (positiveConvMod (p - y2) p) = NullPoint
                | otherwise =   let
                                    m = if ((x1==x2) && (y1==y2)) then
                                            modMult ((modMult 3 (modPow x1 2 p) p)+a) 
                                                (inverse (modMult 2 y1 p) p) p
                                        else
                                            modMult dy (inverse dx p) p
                                            where 
                                                dy = positiveConvMod (y2-y1) p
                                                dx = positiveConvMod (x2-x1) p
...
nthMultEllp curve point n p = nthMultEllp' curve NullPoint point n p
...
discLogEllp::EllpCurve->Point->Point->Integer->Integer
discLogEllp curve p q m    = discLogEllp' curve p q m 2
discLogEllp' curve p q m n = let 
                            np = nthMultEllp curve p n m
                         in
                            if np == q then
                                n
                            else
                                discLogEllp' curve p q m (n+1) 
...
modPow x y m | x < 0           = modPow (positiveConvMod x m) y m 
             | y < 0           = modMult (inverse x m) (modPow x (abs y) m) m
             | y <= 1          = mod (x^y) m
